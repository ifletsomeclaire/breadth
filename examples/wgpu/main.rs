...
impl WideInstance {
    ...
    pub fn push(&mut self, position: &Vec3) -> Index {
...
impl State {
    async fn new(window: &Window) -> Self {
        ...
        let positions = instances
            .iter()
            .map(|i| {
                let pos: [f32; 3] = i.position.into();
                Vec3::from(pos)
            })
            .collect::<Vec<_>>();
        let mut wide_instances = WideInstance::new(1);
        for pos in positions.iter() {
            wide_instances.push(pos);
        }

        // let instance_data = instances.iter().map(Instance::to_raw).collect::<Vec<_>>();
        let instance_data = wide_instances.to_raw();
    ...
    fn update(&mut self) {
        ...
        self.wide_instances.calculate(f32x8::splat(0.01));
        let instance_data = self.wide_instances.to_raw();
        self.queue.write_buffer(
